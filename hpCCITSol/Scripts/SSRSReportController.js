...
app.controller("SSRSReportController", ['$scope', '$http', '$uibModal', '$log', '$rootScope', 'crudService', 'crudServiceController', '$q', 'uiGridConstants', '$timeout', '$filter', '$templateCache', 'NgTableParams', 'ngNotify', function ($scope, $http, $uibModal, $log, $rootScope, crudService, crudServiceController, $q, uiGridConstants, $timeout, $filter, $templateCache, NgTableParams, ngNotify) {
    ...
    function startup() {
        ...
        //ui-grid 
            ...
            var myEditAndDeleteCellTemplate = '<div class="text-center" style="padding-top: 3px;"><button type="button" class="btn btn-info btn-xs" ng-click="grid.appScope.ShowDetails(row)">Edit <span class="glyphicon glyphicon-edit"></span></button>&nbsp;' +
                                           ' <button type="button" class="btn btn-danger btn-xs" ng-click="grid.appScope.Delete(row)">Delete <span class="glyphicon glyphicon-remove"></span></button>&nbsp;' +
                                            ' <button type="button" class="btn btn-success btn-xs" ng-click="grid.appScope.ShowReportScheduler(row.entity.ssrs_report_id)">Scheduler</button> </div>';
            ...
            $scope.gridOptions1 = {
                ...
                columnDefs: [
               { name: 'View', field: 'ssrs_report_id', cellTemplate: myEditAndDeleteCellTemplate, enableFiltering: false, width: "250" },
               { name: 'Report Name', field: 'ssrs_report_nm' },
               //{ name: 'Description', field: 'report_descr', width: "450" }
                ...
                onRegisterApi: function (gridApi) {
                    ...
                    gridApi.selection.on.rowSelectionChanged($scope, function (row) {
                        $scope.ReportDetails(row.entity.ssrs_report_id);
            ...
            $scope.ShowReportScheduler= function (ssrs_report_id) {
                $scope.GetDetail(ssrs_report_id, 'scheduler');
            }
        ...
        $scope.ShowDetails = function (row) {
            $scope.IsEdit = true;
            $scope.GetDetail(row.entity.ssrs_report_id);
        }
        ...
        $scope.openDialogConfirm = function (row, headerText, bodyText, closeButtonText, actionButtonText) {
            ...
            uibModalInstance.result.then(function (updateddata) {
                ...
                if (updateddata) {
                    var promisePut = crudServiceController.delete(row.entity.ssrs_report_id, CONTROLLERNAME).then(function (pl) {
    ...
    //Report- Dialog
        ...
        $scope.ValidateData = function () {
            ...
            function Save() {
                ...
                var promisePut;
                if ($scope.data.ssrs_report_id== 0) //Add
                {
                    promisePut = crudServiceController.post($scope.data, CONTROLLERNAME); 
                }
                else //Update
                {
                    promisePut = crudServiceController.put($scope.data.ssrs_report_id, $scope.data, CONTROLLERNAME);
                }
            ...
            $scope.Delete = function (row) {
                var bodyText = 'ID : ' + row.entity.ssrs_report_id + ' : ' + row.entity.report_nm + '  Are you sure to delete?';
    ...
    // Function to load all Configurations records
            ...
            $scope.ReportDetails = function (ssrs_report_id) {

                $scope.selecteddata.ssrs_report_id = ssrs_report_id;

                var filterparameters = {
                    startdate: $rootScope.dates.startdate,
                    enddate: $rootScope.dates.enddate,
                    ssrs_report_id: ssrs_report_id
                };
            ...
            $scope.EnableDisableSchedulingTimer = function () {
                ...
                var filterparameters = {
                    startdate: $rootScope.dates.startdate,
                    enddate: $rootScope.dates.enddate,
                    ssrs_report_id: $scope.selecteddata.ssrs_report_id,
